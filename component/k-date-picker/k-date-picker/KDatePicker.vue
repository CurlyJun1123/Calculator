...
export default {
  ...
  computed: {
    ...
    blocks() {
      const dates = this.type === 'day' ? this.days : this.type === 'month' ? this.months : this.years
      const result = []
      const chineseMap = {
        day: (date) => {
      ...
      for (const date of dates) {
        if (!date) {
          ...
          const obj = {
            ...
            isStart: this.rangeStart && date.startOf(this.type).isSame(this.rangeStart.startOf(this.type)),
            isEnd: this.rangeEnd && date.startOf(this.type).isSame(this.rangeEnd.startOf(this.type)),
            isInRange:
              this.rangeStart &&
              this.rangeEnd &&
  ...
  methods: {
    onClick() {
      if (this.isRange) {
        const _rangeStart = this.rangeStart ? (this.formatter ? this.rangeStart.format(this.formatter) : this.rangeStart.valueOf()) : null
        ...
        const _checked =
          this.selectedDate !== '-'
            ? this.multiple
              ? this.checked.map((item) => (this.formatter ? item.format(this.formatter) : item.valueOf()))
              : this.formatter
    ...
    toggle(type, direction) {
      if (this.type === 'day') {
        this.currentDay = this.currentDay.add(direction === 'left' ? -1 : 1, type === 'single' ? 'month' : 'year')
      }
      if (this.type === 'month') {
    ...
    init() {
      ...
      if (this.isRange) {
        ...
        this.currentDay = this.defaultValue[0] ? dayjs(this.defaultValue[0]) : dayjs()
...
<template>
  <KBottomPopup v-model="show">
    <view class="k-date-picker">
      ...
      <view class="inner-body">
        <view class="inner-body__arrow">
          <image v-if="type === 'day'" class="image-arrow" :src="left_double" @click="toggle('double', 'left')" />
          ...
          <image v-if="type === 'day'" class="image-arrow right-arrow" :src="left_double" @click="toggle('double', 'right')" />
        </view>
        <view class="inner-body__calendar">
          <view class="inner-body__calendar_week" :style="{ columnGap: isRange ? '0' : '12rpx' }" v-if="type === 'day'">
            <view class="inner-body__calendar_week--item" v-for="(item, index) in weeksChinese" :key="index">{{ item }}</view>
          </view>
          <view
    ...
    <KButton :label="confirmText" :box-shadow="false" :safe-area="true" @onClick="onClick"></KButton>
  </KBottomPopup>
</template>
