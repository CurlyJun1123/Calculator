<template>
  <view
    ...
    :style="{ paddingBottom: safeArea && `${bottomSafeArea || 10}px`, boxShadow: boxShadow ? '0 2px 12px 0 rgba(0, 0, 0, 0.1)' : '' }"
  >
    <button
      v-for="(button, index) in list"
      class="k-button"
      :style="{ color: button.color, backgroundColor: button.background, border: `2rpx solid ${button.borderColor || 'transparent'}` }"
      :key="index"
      @click="onClick(index)"
    >
      {{ button.label }}
