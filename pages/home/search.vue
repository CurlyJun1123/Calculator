<template>
  <view class="page-cell">
    <navigator v-for="(item, index) in list" v-bind:key="index" :url="'/pages/home/detail?id=' + item.id" open-type="navigate" hover-class="navigator-hover">
          ...
          <view class="page-info">
            ...
            <view class="page-info-content">
              <view class="page-info-score-sold">
                <view class="page-info-score">{{ item.score }}分</view>
...
export default {
  ...
  methods: {
    getListData(projectType) {
      this.$http.get('/hy/project/list', { params: { pageNum: 1, pageSize: 10, projectType } }).then((data) => {
        this.list = data.rows
      })
    }
  }
